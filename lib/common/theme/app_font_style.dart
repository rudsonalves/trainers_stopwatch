import 'package:flutter/material.dart';

class AppFontStyle {
  AppFontStyle._();

  static const ibm30 = TextStyle(
    fontSize: 30,
    fontFamily: 'IBMPlexMono',
  );

  static const ibm26 = TextStyle(
    fontSize: 26,
    fontFamily: 'IBMPlexMono',
  );

  static const ibm18 = TextStyle(
    fontSize: 18,
    fontFamily: 'IBMPlexMono',
  );

  static const ibm14 = TextStyle(
    fontSize: 14,
    fontFamily: 'IBMPlexMono',
  );

  static const ibm14SemiBold = TextStyle(
    fontSize: 14,
    fontWeight: FontWeight.w600,
    fontFamily: 'IBMPlexMono',
  );

  static const roboto12 = TextStyle(
    fontSize: 12,
    // fontFamily: 'Roboto',
  );

  static const roboto12Bold = TextStyle(
    fontSize: 12,
    fontWeight: FontWeight.w600,
    // fontFamily: 'Roboto',
  );

  static const roboto14 = TextStyle(
    fontSize: 14,
    fontWeight: FontWeight.w400,
    // fontFamily: 'Roboto',
  );

  static const roboto14SemiBold = TextStyle(
    fontSize: 12,
    fontWeight: FontWeight.w600,
    // fontFamily: 'Roboto',
  );

  static const roboto14Bold = TextStyle(
    fontSize: 12,
    fontWeight: FontWeight.w700,
    // fontFamily: 'Roboto',
  );

  static const roboto18SemiBold = TextStyle(
    fontSize: 18,
    fontWeight: FontWeight.w600,
  );

  static const roboto20SemiBold = TextStyle(
    fontSize: 20,
    fontWeight: FontWeight.w600,
  );

  static const roboto16 = TextStyle(
    fontSize: 16,
  );

  static const roboto16SemiBold = TextStyle(
    fontSize: 16,
    fontWeight: FontWeight.w600,
  );

  static const roboto10 = TextStyle(
    fontSize: 10,
  );
}
