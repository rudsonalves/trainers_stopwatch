...
import '/ui/pages/stopwatch/bloc/stopwatch_state.dart';
...
class StopwatchButtonBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<StopwatchBloc, StopwatchState>(
        ...
        builder: (context, state) =>
            _buildRow(_createIconButtonList(ctx: context, state: state)));
  }

  List<CustomIconButton> _createIconButtonList({
    required BuildContext ctx,
    required StopwatchState state,
  }) {
    ...
    return switch (state.status) {
      ...
      StopwatchStatus.running => [
          state.splitCount == state.splitsPerLap - 1
              ? CustomIconButton(
                  onPressed:
                      session.isOperationRunning ? null : () => session.lap(),
                  label: 'PSLaps'.tr(),
                  icon: Icon(StopwatchIcons.lap1, color: onSurfaceVariant),
                )
              : CustomIconButton(
                  onPressed:
                      session.isOperationRunning ? null : () => session.split(),
                  label: 'PSSplit'.tr(),
                  icon: Icon(StopwatchIcons.partial, color: onSurfaceVariant),
                ),
          CustomIconButton(
            onPressed:
  ...
  Row _buildRow(List<CustomIconButton> buttons) {
    return Row(
